...
impl<'a> DefragmentationContext<'a> {
  ...
  pub fn begin_pass(&self, mover: impl FnOnce(&mut [DefragmentationMove]) -> ()) -> bool {
    ...
    let moves =
      unsafe { core::slice::from_raw_parts_mut(pass_info.pMoves, pass_info.moveCount as usize) };
    mover(moves);

    let result =
      unsafe { ffi::vmaEndDefragmentationPass(self.allocator.internal, self.raw, &mut pass_info) };
