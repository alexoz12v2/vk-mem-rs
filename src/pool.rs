use core::{ffi::CStr, ops::Deref, ptr::null_mut, mem::ManuallyDrop};
...
pub trait Alloc {
  ...
  unsafe fn allocate_memory_pages(
  ...
  ) -> VkResult<alloc::vec::Vec<Allocation>> {
    let mut create_info: ffi::VmaAllocationCreateInfo = create_info.into();
    create_info.pool = self.pool();
    let mut allocations: alloc::vec::Vec<ffi::VmaAllocation> =
      vec![core::mem::zeroed(); allocation_count];
    ...
    let allocations: alloc::vec::Vec<Allocation> = allocations
      .into_iter()
  ...
  unsafe fn create_buffer_get_info(
    ...
    Ok((
      ...
      AllocationInfo::from(alloc_info),
    ))
  }
  ...
  unsafe fn create_image_with_alloc_info(
    ...
    Ok((
      image,
      Allocation(allocation),
      AllocationInfo::from(allocation_info),
    ))
  }
}
