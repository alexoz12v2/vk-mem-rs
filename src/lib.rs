...
impl AllocatorView {
    ...
    pub fn build_stats_string(&self, detailed_map: bool) -> VkResult<alloc::string::String> {
        unsafe {
            let mut p_stats_string: *mut core::ffi::c_char = core::ptr::null_mut();
            ffi::vmaBuildStatsString(
                self.internal,
                &mut p_stats_string,
                if detailed_map { 1 } else { 0 },
            );
            if p_stats_string.is_null() {
                return Err(ash::vk::Result::ERROR_UNKNOWN);
            }
            let c_str = core::ffi::CStr::from_ptr(p_stats_string);
            let result_str = c_str.to_string_lossy().into_owned();
            ffi::vmaFreeStatsString(self.internal, p_stats_string);
            Ok(result_str)
        }
    }
    ...
    pub fn get_heap_budgets(&self) -> VkResult<alloc::vec::Vec<ffi::VmaBudget>> {
        unsafe {
            ...
            let mut vma_budgets: alloc::vec::Vec<ffi::VmaBudget> =
